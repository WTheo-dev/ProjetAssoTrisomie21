<?php
	...
	switch($_POST) {
		case isset($_POST["submit_intitule"]):
			$maReq = 'UPDATE récompense SET Intitulé=:intitule WHERE Intitulé = :intitule_modif;';
		...
		case isset($_POST["submit_des"]):
			$maReq = 'UPDATE récompense SET Descriptif=:descriptif WHERE Intitulé = :intitule_modif;';
			$wesh = array('intitule_modif' => $_POST['intitule_modif'], 'descriptif' => $_POST["descriptif"]);
			prepare_and_execute($maReq, $wesh);
			break;
		case isset($_POST["submit_img"]):
			$maReq = 'UPDATE récompense SET Lien_Image=:lien WHERE Intitulé = :intitule_modif;';
	...
	header("Location: modifierRecompensePage.php");
